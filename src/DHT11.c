...
unsigned short int leituraByteDht11(){
  ...
  //O padrao do DHT11 � 50us entre bits
  //BIT 1 � linha  de dados em vcc por 70us
  //BIT 0 � a linha de dados em GND por 25us
...
void leituraDht11(unsigned short int *dht11_humi, unsigned short int *dht11_temp){
  ...
  startTimer1();
  // Inicializa��o do DHT
  while( DHT11_PIN  && timer1 <= 1);  //AGUARDA INCIALIZA��O DO DHT
  while( !DHT11_PIN && timer1 <= 1);
  while( DHT11_PIN && timer1 <= 1);
  //Stop TIMER 1
  stopTimer1();
  // Leitura das infomra��es
  if(timer1 == 0){
  ...
  *dht11_temp = leituraByteDht11();  // LEITURA DO PRIMEIRO BYTE DE TEMPERATURA 1
  leituraByteDht11();               // LEITURA DO SEGUNDO BYTE TEMPERATURA
  leituraByteDht11();               // LEITURA DO DADO DE VERIFI��O
  }else{
  *dht11_humi= 0;
  *dht11_temp= 99;
  }
